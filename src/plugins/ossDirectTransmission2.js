...
export function _ossDT(file, callback) {
  ...
  let pro = new Promise((resolve, rej) => {
    ...
    Object.values(fileList).forEach((item, index) => {
      uploadImgArr.push({
        info: upFile(item, data,callback)
      });
    });
    callback(uploadImgArr);
  });
}
function upFile(item, data, callback) {
  ...
  // console.log(ossData.get("key"),'----------------116')
  // console.log(ossData.get("policy"), "----------------117");
  // console.log(ossData.get("OSSAccessKeyId"), "----------------118");
  // console.log(ossData.get("success_action_status"), "----------------119");
  // console.log(ossData.get("signature"), "----------------120");
  // console.log(ossData.get("file"), "----------------121");
  let val = {
    type: "conduct",
    res: {},
    progress: 0
